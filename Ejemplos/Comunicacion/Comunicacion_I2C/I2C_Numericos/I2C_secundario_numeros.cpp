/*ESTO ES LO MISMO QUE EL OTRO EJEMPLO PERO EN VEZ DE TEXTO MANDAMOS NUMEROS*/
//Codigo reestructurado por @A.N.A

#include <Arduino.h>
#include <Wire.h>

#define I2C_ADDR_SLAVE_A 0x55  // Dirección del esclavo (ESP32-2)
#define I2C_ADDR_SLAVE_B 0x66  // Dirección del otro esclavo (ESP32-1)
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("=== ESP32-2 (Esclavo) ===");
...
void loop() {
    ...
    if (I2C_bus_A.available()) {
        ...
        Serial.print("[Master<-Slave ESP32-1] Respuesta: ");
        Serial.println(response);
    }
    Serial.println("--------------------------------------------");
}
