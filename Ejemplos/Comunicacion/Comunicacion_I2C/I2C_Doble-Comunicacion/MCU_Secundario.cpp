...
/*- En este caso estamos asignando los pines 21 y 22 como los maestros de esa comunicacion*/
...
void onReceiveB(int len) {
  ...
  //Mientras el bus esta recibiendo datos junta todos estos en un char llamado c
  while (I2C_bus_B.available()) {
    char c = I2C_bus_B.read();
    received += c;
  }

  //imprimimos el dato que recibimos en el serial 
  Serial.println("[Slave ESP32-2] Recibido desde ESP32-1: " + received);
}
...
/* 
  ...
  snprintf(response, sizeof(response), "%lu de ESP32-2", msgCountB++);
...
void setup() {
  ...
  ================================================================
  En este caso es el maestro y esta usando los pines 21 y 22*/

  I2C_bus_A.begin(SDA_MASTER, SCL_MASTER);
  Serial.println("I2C Maestro (A) iniciado en SDA 21, SCL 22");
  ...
  if (success) { // si se inicio el esclavo
    ...
    Serial.println("I2C Esclavo (B) iniciado en SDA 25, SCL 26");
...
void loop() {
  ...
  // Enviar mensaje al esclavo ESP32-1
  String message = "Hola desde ESP32-2";
  ...
  Serial.println("[Master -> Slave ESP32-1] Enviado: " + message);
  ...
  I2C_bus_A.requestFrom(I2C_ADDR_SLAVE_B, 32);
  ...
  Serial.println("[Master <- Slave ESP32-1] Respuesta recibida: " + response);
  Serial.println("--------------------------------------------");
}
